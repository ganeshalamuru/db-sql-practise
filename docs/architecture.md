...
The exercise catalog is the source of truth. `scripts/build_problems.py` converts its structured metadata into student-facing Markdown and `exercise_registry.json`, which the grader consumes. Answers and instructor-only solutions are both organized in matching topic folders, making each module self-contained. Adding an exercise means adding one catalog object, one instructor-only solution, and rerunning the build script.
